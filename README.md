...
> ⚠️ This project is designed to run in **Jupyter Notebook (.ipynb)** and was executed in a **GPU environment**.
...
.
├── README.md
├── final_code.ipynb         # Main training + inference pipeline (produces final submission)
└── gpt_usage.ipynb          # GPT-based strong silver label + class description generation
...
✅ After downloading your dataset, **change `DATA_BASE`** to your local directory, e.g.:
...
1. **Class descriptions** → used to build taxonomy-aware class embeddings
2. **Strong silver labels** → used to train seed models on high-confidence pseudo labels
